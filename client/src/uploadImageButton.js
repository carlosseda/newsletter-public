...
class UploadImageButton extends HTMLElement {
    ...
    constructor() {
        ...
        this.createThumbnailHandler = event => {
        ...
        this.updateThumbnailHandler = event => {
            if(event.detail.image.name === this.getAttribute('name') && event.detail.image.languageAlias === this.getAttribute('languageAlias')){
                this.updateThumbnail(event.detail.image, event.detail.previousImage);
            }
        };

        this.showThumbnailsHandler = event => {
            this.showThumbnails(event.detail.images);
        };

        this.deleteThumbnailsHandler = event => {
            this.deleteThumbnails();
        };

        document.addEventListener('showThumbnails', this.showThumbnailsHandler);
        document.addEventListener('createThumbnail', this.createThumbnailHandler);
        document.addEventListener('updateThumbnail', this.updateThumbnailHandler);
        document.addEventListener('deleteThumbnails', this.deleteThumbnailsHandler);
    }
    ...
    disconnectedCallback() {
        document.removeEventListener('showThumbnails', this.showThumbnailsHandler);
        document.removeEventListener('createThumbnail', this.createThumbnailHandler);
        document.addEventListener('updateThumbnail', this.updateThumbnailHandler);
        document.addEventListener('deleteThumbnails', this.deleteThumbnailsHandler);
    }
    ...
    async createThumbnail(image) {
        ...
        file.src = `${API_URL}/api/admin/image-gallery/${image.filename}`;
    ...
    async updateThumbnail(image, previousImage) {
        ...
        if(this.shadow.querySelector(`.upload-image[data-filename="${previousImage}"]`)){
            ...
            let thumbnail = this.shadow.querySelector(`.upload-image[data-filename="${previousImage}"]`);

            thumbnail.querySelector('img').src = `${API_URL}/api/admin/image-gallery/${image.filename}`;
